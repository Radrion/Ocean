...
def ask_ollama(messages):
    """Single place that calls Ollama — used by both sql_translate() and
    summarize() so there's only one spot to update model params later."""
...
def summarize(user_question, sql_query, result_df, was_fallback=False):
    ...
    return ask_ollama([{"role": "user", "content": prompt}])
